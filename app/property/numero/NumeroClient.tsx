...
async function fetchAndInjectCSS(backendUrl: string, theme: 'light' | 'dark' = 'light'): Promise<void> {
  ...
  try {
    ...
    // Global CSS (theme-specific)
    if (theme === 'light' && settings.global_css_light) {
      cssToInject += `\n/* Global CSS - Light */\n${settings.global_css_light}\n`;
    } else if (theme === 'dark' && settings.global_css_dark) {
      cssToInject += `\n/* Global CSS - Dark */\n${settings.global_css_dark}\n`;
    }
    
    // Legacy light/dark CSS fields
    if (theme === 'light' && settings.light_css) {
      cssToInject += `\n/* Light Theme CSS */\n${settings.light_css}\n`;
    } else if (theme === 'dark' && settings.dark_css) {
      cssToInject += `\n/* Dark Theme CSS */\n${settings.dark_css}\n`;
    }
    
    // Breakpoint CSS (theme-specific)
    if (theme === 'light' && settings.breakpoint_css_light) {
      cssToInject += `\n/* Breakpoint CSS - Light */\n${settings.breakpoint_css_light}\n`;
    } else if (theme === 'dark' && settings.breakpoint_css_dark) {
      cssToInject += `\n/* Breakpoint CSS - Dark */\n${settings.breakpoint_css_dark}\n`;
    }
    
    // Typography CSS (applies to both themes)
    if (settings.typography_css) {
      cssToInject += `\n/* Typography CSS */\n${settings.typography_css}\n`;
    }
    
    // Inject CSS if we have any
    if (cssToInject.trim()) {
    ...
    console.warn('Could not fetch Numero CSS settings:', e);
  }
}
...
const formatLocation = (neighbourhoodObj: any, property: any) => {
  ...
  if (city && city.trim() !== '' && city.toUpperCase() !== 'NONE') {
    // Don't add city if it's the same as neighbourhood
...
// Calculates the correct section number based on which conditional sections exist
interface SectionNumbers {
  story: string;        // 01 - always present
  gallery: string;      // 02 - always present (if images)
  virtualTour: string;  // Conditional
  features: string;     // Conditional
  roomDimensions: string; // Conditional
  neighbourhood: string;  // First neighbourhood section (hero/intro combined)
  map: string;          // Interactive map
  outside: string;      // Get Outside
  amenities: string;    // Local Amenities
  shopDine: string;     // Shop & Dine
}
...
const getSectionNumbers = (property: any, hasNeighbourhoodOrCity: boolean, profileConfig: any): SectionNumbers => {
  ...
  // 01 - The Story (always)
  const story = num.toString().padStart(2, '0');
  num++;
  
  // 02 - Photo Gallery (always if images exist)
  const gallery = num.toString().padStart(2, '0');
  if (property?.images && property.images.length > 0) num++;
  
  // Virtual Tour (conditional)
  let virtualTour = '';
  if (property?.virtual_tour_url) {
    virtualTour = num.toString().padStart(2, '0');
    num++;
  }
  
  // Features & Amenities (conditional)
  let features = '';
  const hasFeatures = (
    ...
    (property?.community_features && property.community_features.length > 0)
  );
  if (hasFeatures) {
    features = num.toString().padStart(2, '0');
    num++;
  }
  
  // Room Dimensions (conditional)
  let roomDimensions = '';
  let hasRoomDimensions = false;
  if (property?.room_dimensions) {
    try {
      const rooms = typeof property.room_dimensions === 'string' 
        ? JSON.parse(property.room_dimensions) 
        : property.room_dimensions;
      if (Array.isArray(rooms) && rooms.length > 0) {
        hasRoomDimensions = true;
        roomDimensions = num.toString().padStart(2, '0');
        num++;
      }
    } catch (e) {}
  }
  
  // Neighbourhood sections (only if neighbourhood or city exists)
  let neighbourhood = '';
  let map = '';
  let outside = '';
  let amenities = '';
  ...
  if (hasNeighbourhoodOrCity) {
    ...
    const hasHeroBanner = profileConfig?.heroBanner?.heroImage;
    const hasNeighbourhoodIntro = profileConfig?.neighbourhoodIntro;
    const hasOutside = profileConfig?.outside?.images?.length > 0;
    const hasAmenities = profileConfig?.amenities?.images?.length > 0;
    const hasShopDine = profileConfig?.shopDine?.images?.length > 0;
    
    // Neighbourhood intro - only show if neighbourhoodIntro config exists
    if (hasNeighbourhoodIntro) {
      neighbourhood = num.toString().padStart(2, '0');
      num++;
    }
    
    // Interactive Map - always show if neighbourhood/city exists (even without other sections)
    map = num.toString().padStart(2, '0');
    num++;
    
    // Get Outside (conditional)
    if (hasOutside) {
      outside = num.toString().padStart(2, '0');
      num++;
    }
    
    // Local Amenities (conditional)
    if (hasAmenities) {
      amenities = num.toString().padStart(2, '0');
      num++;
    }
    
    // Shop & Dine (conditional)
    if (hasShopDine) {
      shopDine = num.toString().padStart(2, '0');
      num++;
    }
  }
...
// Animation component for fade-in on scroll
...
// Animated Headline - LEFT to RIGHT character reveal (like redox GSAP charAnimation)
// Characters fade in from x:20 to x:0 with stagger, creating left-to-right reveal
const AnimatedHeadline = ({ text, isDark = false }: { text: string; isDark?: boolean }) => {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: '-50px' });
  
  // Split into characters but keep word boundaries for proper wrapping
...
// Hero Image - Grows from 60% to 100% WIDTH on scroll (like redox growAnimation)
// Uses requestAnimationFrame for smooth 60fps animation and lerp for smoothness
const GrowHeroImage = ({ src, alt }: { src: string; alt: string }) => {
  ...
  useEffect(() => {
    ...
    const updateWidth = () => {
      // Lerp (linear interpolation) for smooth animation - similar to GSAP scrub: 2
      ...
      if (containerRef.current) {
        containerRef.current.style.width = `${widthRef.current}%`;
      }
    ...
    const handleScroll = () => {
      ...
      // When element top is at bottom of viewport (90%), progress = 0 (60% width)
      // When element top is at center of viewport (50%), progress = 1 (100% width)
      // This works BOTH directions - scrolling down increases, scrolling up decreases
      ...
      // Width from 60% to 100% based on scroll position
...
// Floor Plan CTA Component - cta-area-4 style with underline animation
const FloorPlanCTA = ({ url, textColor = '#1a1a1a' }: { url: string; textColor?: string }) => {
  const [isHovered, setIsHovered] = useState(false);
...
// Masonry Gallery Component - 8 images on desktop, 5 on tablet, 3 on mobile
// With cta-area-4 animated link (underline slides left, arrow animates)
const MasonryGallery = ({ images, onImageClick, showMoreText = 'Show more property photos' }: { images: string[]; onImageClick: (index: number) => void; showMoreText?: string }) => {
  ...
  const parseShowMoreText = (text: string) => {
    // Default parsing: "Show more X photos" -> "See more" + "X photos"
  ...
  return (
    <div className="numero-masonry-gallery">
      <style jsx>{`
        .numero-masonry-gallery {
          display: grid;
        ...
        .numero-masonry-gallery .gallery-item { min-height: 200px; cursor: pointer; overflow: hidden; border-radius: 12px; }
        .numero-masonry-gallery .gallery-item img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.5s ease; }
        .numero-masonry-gallery .gallery-item:hover img { transform: scale(1.05); }
        .numero-masonry-gallery .gallery-item:nth-child(1) { grid-area: a; }
        .numero-masonry-gallery .gallery-item:nth-child(2) { grid-area: b; }
        .numero-masonry-gallery .gallery-item:nth-child(3) { grid-area: c; }
        .numero-masonry-gallery .gallery-item:nth-child(4) { grid-area: d; }
        .numero-masonry-gallery .gallery-item:nth-child(5) { grid-area: e; }
        .numero-masonry-gallery .gallery-item:nth-child(6) { grid-area: f; }
        .numero-masonry-gallery .gallery-item:nth-child(7) { grid-area: g; }
        .numero-masonry-gallery .gallery-item:nth-child(8) { grid-area: h; }
        @media (max-width: 900px) {
          ...
          .numero-masonry-gallery .gallery-item:nth-child(n+6) { display: none; }
        }
        @media (max-width: 600px) {
          ...
          .numero-masonry-gallery .gallery-item:nth-child(n+4) { display: none; }
        }
        /* CTA Area 4 - See More Styles - IDENTICAL to redox */
        .numero-cta-area-4 {
          grid-column: 1 / -1;
          width: 100%;
          margin-top: 40px;
        ...
        .numero-cta-area-4 .cta-title::before {
          position: absolute;
          content: "";
          left: 0;
          bottom: 0;
        ...
        .numero-cta-area-4 .cta-title:hover::before {
          width: 0;
        }
        .numero-cta-area-4 .cta-title .icon-wrapper {
          margin-left: 16px;
          display: inline-block;
          line-height: 0;
          position: relative;
        ...
        .numero-cta-area-4 .cta-title .icon-wrapper .icon-first {
          transition: all 0.3s;
          width: 100%;
          height: 100%;
        }
        .numero-cta-area-4 .cta-title .icon-wrapper .icon-second {
          position: absolute;
          bottom: 0;
          left: 0;
        ...
        .numero-cta-area-4 .cta-title:hover .icon-wrapper .icon-first {
          transform: translate(100%, -100%);
        }
        .numero-cta-area-4 .cta-title:hover .icon-wrapper .icon-second {
          transform: translate(0%, 0%);
        }
      `}</style>
      {displayImages.map((img, idx) => (
        <div key={idx} className="gallery-item" onClick={() => onImageClick(idx)}>
          <img src={img} alt={`Photo ${idx + 1}`} />
        </div>
      ))}
      {hasMore && (
        <div className="numero-cta-area-4">
...
// Image Lightbox - Light theme: WHITE background, styled like default template
const ImageLightbox = ({ images, index, setIndex, onClose }: { images: string[]; index: number; setIndex: (i: number) => void; onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 bg-white z-[9999] flex items-center justify-center" tabIndex={0}>
      <button onClick={onClose} className="absolute top-4 right-4 text-gray-900 hover:text-gray-600 z-10 w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6L6 18M6 6l12 12" /></svg>
      </button>
      <button onClick={(e) => { e.stopPropagation(); setIndex((index - 1 + images.length) % images.length); }} className="absolute left-4 text-gray-900 hover:text-gray-600 p-2 w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center">
      ...
      <button onClick={(e) => { e.stopPropagation(); setIndex((index + 1) % images.length); }} className="absolute right-4 text-gray-900 hover:text-gray-600 p-2 w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center">
...
interface NumeroClientProps {
  propertyIdProp?: string;
  agentIdProp?: string;
}

export default function NumeroClient({ propertyIdProp, agentIdProp }: NumeroClientProps = {}) {
  const searchParams = useSearchParams();
  // Use prop if provided, otherwise fall back to URL params
  const propertyId = propertyIdProp || searchParams.get('id');
  ...
  useEffect(() => {
    if (BACKEND_URL) {
      fetchAndInjectCSS(BACKEND_URL, 'light');
    }
  }, [BACKEND_URL]);
      ...
      try {
        ...
        if (backendUrl && mlsNumber) {
          try {
            ...
            if (apiRes.ok) {
              ...
              // Set property data from API
              const p = apiData.property || {};
              p.images = rawImages ? rawImages.map((url: string) => typeof url === 'string' ? { url } : url) : [];
              setProperty(p);
              
              // Set content from API (this is the properly merged content)
              setContent(apiData.content || {});
              
              // Set neighbourhood from API
              setNeighbourhood(apiData.neighbourhood || apiData.neighborhood || null);
              
              // Set city from API (for neighbourhood fallback)
              if (apiData.city) {
                setCity(apiData.city);
              } else if (!apiData.neighbourhood && !apiData.neighborhood && p.city) {
                // No neighbourhood - try to fetch city profile as fallback
                try {
                  const cityRes = await fetch(`${backendUrl}/api/city-profile/${encodeURIComponent(p.city)}`);
                  if (cityRes.ok) {
                  ...
                  console.log('No city profile found for:', p.city);
                }
              }
            }
          } catch (apiErr) {
        ...
        if (coAgentIds?.length > 0) {
          ...
          if (coAgents?.length > 0) {
            setCoListingAgents(coAgents.map((a: any) => ({ id: a.id, name: a.full_name || a.name, phone: a.phone_number, email: a.email, profile_image: a.profile_image_url })));
          }
        }
        
        // Fetch neighbourhood if not from API
  ...
  // Check conditions for nav items
  const hasVirtualTour = useMemo(() => {
    if (!property) return false;
    return !!(property.virtual_tour_url || property.virtual_tour_link);
  }, [property]);

  const hasNeighbourhood = useMemo(() => {
    return !!(neighbourhood && (neighbourhood.name || neighbourhood.id));
  }, [neighbourhood]);

  // Check if we have city data (fallback when no neighbourhood)
  const hasCity = useMemo(() => {
    return !!(city && (city.name || city.id));
  }, [city]);

  // Use neighbourhood OR city for profile sections
  ...
  const navItems: NavItem[] = useMemo(() => {
    ...
    if (hasVirtualTour) {
      items.push({ id: '360-tour', label: '360 Tour', sectionId: 'virtual-tour-section' });
    }
  ...
  const getAgentButtonText = useCallback(() => {
    ...
    if (coListingAgents.length > 0 && coListingAgents[0]?.name) {
      const coFirstName = coListingAgents[0].name.split(' ')[0];
      return `Contact ${primaryFirstName} & ${coFirstName}`;
    }
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: COLORS.bg }}><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-black" /></div>;
  if (!property) return <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: COLORS.bg }}><h1 className="text-2xl font-bold">Property not found</h1></div>;
  ...
  return (
    <>
      {/* Sequel Sans Font - Local */}
      <style jsx global>{`
        @font-face {
          font-family: 'Sequel Sans';
          src: url('/assets/fonts/Sequel Sans Medium Body.otf') format('opentype');
          font-weight: 500;
          font-style: normal;
          font-display: swap;
        }
        @font-face {
          font-family: 'Sequel Sans';
          src: url('/assets/fonts/Sequel Sans Roman Body.otf') format('opentype');
          font-weight: 400;
          font-style: normal;
          font-display: swap;
        }
        .numero-light, .numero-light * {
          font-family: 'Sequel Sans', 'Inter', -apple-system, BlinkMacSystemFont, sans-serif !important;
        }
        
        /* Prose paragraph margins - ensure spacing between paragraphs */
        .numero-light .prose p,
        .numero-light [class*="prose"] p {
          margin-bottom: 1.25em;
        }
        .numero-light .prose p:last-child,
        .numero-light [class*="prose"] p:last-child {
          margin-bottom: 0;
        }
        
        /* Hover effects */
        .numero-nav-agent-btn:hover { background: #e5e7eb !important; }
        .numero-nav-agent-btn:hover .numero-nav-arrow { transform: rotate(45deg); }
        .numero-nav-tour-btn:hover { background: #333 !important; }
        .numero-nav-tour-btn:hover .numero-nav-arrow { transform: rotate(45deg); }
        .numero-nav-item:hover { background: rgba(255,255,255,0.7) !important; color: #1f2937 !important; }
        ...
        .offcanvas-contact-btn:hover { background: #e5e7eb !important; }
        
        /* Modal input focus */
        .numero-modal-content input:focus, .numero-modal-content textarea:focus {
          outline: none; border-color: #000 !important; box-shadow: 0 0 0 2px rgba(0,0,0,0.1);
        }
        
        /* Desktop 1200px+ */
        @media (min-width: 1200px) {
          .numero-agent-text { display: inline !important; }
          .numero-hamburger-btn { display: none !important; }
        }
        /* Tablet 768px-1199px */
        @media (max-width: 1199px) and (min-width: 768px) {
          .numero-agent-text { display: none !important; }
          .numero-nav-center { display: none !important; }
          .numero-hamburger-btn { display: flex !important; }
        }
        /* Mobile below 768px */
        @media (max-width: 767px) {
          .numero-nav-center { display: none !important; }
          .numero-sticky-nav { padding: 8px 12px !important; }
          .numero-nav-agent-btn { padding: 6px !important; }
          .numero-agent-text { display: none !important; }
          .numero-hamburger-btn { display: none !important; }
          .numero-contact-panel, .numero-tour-panel { width: calc(100% - 40px) !important; max-width: none !important; }
        }
        @media (max-width: 575px) {
          .numero-sticky-nav { width: 92% !important; padding: 6px 10px !important; }
          .numero-tour-text { font-size: 13px !important; }
        }
      `}</style>

      {/* Background with shape pattern - WHITE background with clear vertical lines (like redox body-creative-agency) */}
      <div className="fixed inset-0 z-0" style={{ backgroundColor: '#ffffff' }}>
        <div className="absolute inset-0 w-full h-screen" style={{ backgroundImage: 'url(/assets/numero/shape-9-light.webp)', backgroundSize: '100%', backgroundRepeat: 'no-repeat', backgroundPosition: 'top center' }} />
      </div>

      {/* Open House Bar */}
      <OpenHouseBar property={property} theme="light" />

      <div className="numero-light relative z-10 min-h-screen">
        {/* Sticky Navigation Bar - Pill Shaped */}
        <nav 
          className="numero-sticky-nav"
          style={{
            position: 'fixed',
            top: showStickyNav ? '20px' : '-100px',
            left: '50%',
        ...
        >
          ...
          <button
            ...
            className="numero-nav-agent-btn"
            style={{
              display: 'flex',
              alignItems: 'center',
              gap: '10px',
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center' }}>
              {agent?.profile_image ? (
                <img src={agent.profile_image} alt={agent.name || 'Agent'} style={{ width: '36px', height: '36px', borderRadius: '50%', objectFit: 'cover', border: '2px solid white', position: 'relative', zIndex: 2 }} />
              ) : (
                <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#d1d5db', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280', fontWeight: 600, fontSize: '14px', border: '2px solid white', position: 'relative', zIndex: 2 }}>
                  {agent?.name?.charAt(0) || 'A'}
                </div>
              )}
              {coListingAgents.length > 0 && (
                coListingAgents[0]?.profile_image ? (
                  <img src={coListingAgents[0].profile_image} alt={coListingAgents[0].name || 'Co-Agent'} style={{ width: '36px', height: '36px', borderRadius: '50%', objectFit: 'cover', border: '2px solid white', marginLeft: '-12px', position: 'relative', zIndex: 1 }} />
                ) : (
                  <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280', fontWeight: 600, fontSize: '14px', border: '2px solid white', marginLeft: '-12px', position: 'relative', zIndex: 1 }}>
                    {coListingAgents[0]?.name?.charAt(0) || 'A'}
                  </div>
                )
              )}
            </div>
            <span className="numero-agent-text" style={{ fontWeight: 500, fontSize: '14px', color: '#1f2937' }}>
              {getAgentButtonText()}
            </span>
            <div className="numero-nav-arrow" style={{ background: '#000', borderRadius: '50%', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'transform 0.3s ease', flexShrink: 0 }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5"><path d="M7 17L17 7M17 7H7M17 7V17"/></svg>
            </div>
          </button>

          {/* Center - Navigation Items */}
          <div className="numero-nav-center" style={{ display: 'flex', alignItems: 'center', gap: '4px', background: '#f3f4f6', borderRadius: '50px', padding: '4px' }}>
            {navItems.map((item) => (
              <button 
                key={item.id}
                onClick={() => scrollToSection(item.sectionId, item.id)}
                className="numero-nav-item"
                style={{
                  padding: '10px 18px',
                  border: 'none',
                  borderRadius: '50px',
          ...
          <div className="numero-nav-right" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            >
              <span className="numero-tour-text" style={{ fontWeight: 500, fontSize: '14px', color: '#fff' }}>Request Tour</span>
              <div className="numero-nav-arrow" style={{ background: '#fff', borderRadius: '50%', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'transform 0.3s ease', flexShrink: 0 }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="2.5"><path d="M7 17L17 7M17 7H7M17 7V17"/></svg>
              </div>
            </button>
            ...
            <button
              ...
              className="numero-hamburger-btn"
              style={{ display: 'none', width: '44px', height: '44px', background: '#fa2837', border: 'none', borderRadius: '50%', cursor: 'pointer', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '5px', paddingRight: '2px', transition: 'all 0.3s ease' }}
            >
        ...
        <div className={`numero-offcanvas-area ${showMobileMenu ? 'menu-open' : ''}`} style={{ position: 'fixed', top: 0, right: 0, width: showMobileMenu ? '60%' : '0', height: '100%', zIndex: 10001, pointerEvents: showMobileMenu ? 'auto' : 'none' }}>
          <div className="offcanvas-bg" style={{ position: 'fixed', top: '20px', right: '20px', bottom: '20px', width: 'calc(60% - 40px)', borderRadius: '20px', background: 'rgba(255, 255, 255, 0.98)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', boxShadow: '0 20px 30px -8px rgba(19, 19, 22, 0.1)', clipPath: showMobileMenu ? 'circle(150% at calc(100% - 45px) 45px)' : 'circle(0% at calc(100% - 45px) 45px)', transition: 'clip-path 0.7s ease-in-out', zIndex: 2 }} />
          <div className="offcanvas-content" style={{ position: 'relative', zIndex: 3, padding: '100px 60px 60px', height: '100%', overflowY: 'auto', opacity: showMobileMenu ? 1 : 0, transition: 'opacity 0.3s ease 0.3s' }}>
            <button onClick={() => setShowMobileMenu(false)} className="offcanvas-close-btn" style={{ position: 'absolute', top: '30px', right: '30px', width: '50px', height: '50px', background: 'transparent', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '32px', color: '#1f2937', transition: 'transform 0.4s ease' }}>✕</button>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '12px' }}>
              <button onClick={() => { setShowMobileMenu(false); setShowContactModal(true); }} className="offcanvas-contact-btn" style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 20px 12px 12px', background: '#f3f4f6', border: 'none', borderRadius: '50px', cursor: 'pointer', transition: 'background 0.2s ease, opacity 0.3s ease, transform 0.3s ease', opacity: showMobileMenu ? 1 : 0, transform: showMobileMenu ? 'translateY(0)' : 'translateY(20px)', transitionDelay: '0.8s' }}>
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  {agent?.profile_image ? <img src={agent.profile_image} alt={agent.name || 'Agent'} style={{ width: '40px', height: '40px', borderRadius: '50%', objectFit: 'cover', border: '2px solid #ffffff', position: 'relative', zIndex: 2 }} /> : <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280', fontWeight: 600, fontSize: '14px', border: '2px solid #ffffff', position: 'relative', zIndex: 2 }}>{agent?.name?.charAt(0) || 'A'}</div>}
                  {coListingAgents.length > 0 && (coListingAgents[0]?.profile_image ? <img src={coListingAgents[0].profile_image} alt={coListingAgents[0].name || 'Co-Agent'} style={{ width: '40px', height: '40px', borderRadius: '50%', objectFit: 'cover', border: '2px solid #ffffff', marginLeft: '-12px', position: 'relative', zIndex: 1 }} /> : <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280', fontWeight: 600, fontSize: '14px', border: '2px solid #ffffff', marginLeft: '-12px', position: 'relative', zIndex: 1 }}>{coListingAgents[0]?.name?.charAt(0) || 'A'}</div>)}
                </div>
                <span style={{ fontWeight: 500, fontSize: '16px', color: '#1f2937' }}>{getAgentButtonText()}</span>
              </button>
        ...
        {showMobileMenu && <div onClick={() => setShowMobileMenu(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 10000, transition: 'opacity 0.3s ease' }} />}

        {/* Contact Modal - Circular Reveal from LEFT */}
        {showContactModal && <div className="numero-modal-overlay" onClick={() => setShowContactModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 10000 }} />}
        <div className={`numero-contact-panel ${showContactModal ? 'panel-open' : ''}`} style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '90%', maxWidth: '450px', maxHeight: '90vh', borderRadius: '20px', background: 'rgba(255, 255, 255, 0.98)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', boxShadow: '0 20px 30px -8px rgba(19, 19, 22, 0.15)', clipPath: showContactModal ? 'circle(150% at 0% 0%)' : 'circle(0% at 0% 0%)', transition: 'clip-path 0.7s ease-in-out', zIndex: 10001, overflowY: 'auto', pointerEvents: showContactModal ? 'auto' : 'none' }}>
          <div className="numero-modal-content" style={{ padding: '30px', opacity: showContactModal ? 1 : 0, transition: 'opacity 0.3s ease 0.3s' }}>
            <button onClick={() => setShowContactModal(false)} className="contact-panel-close" style={{ position: 'absolute', top: '15px', right: '15px', width: '40px', height: '40px', background: '#f3f4f6', border: 'none', borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', color: '#1f2937', transition: 'transform 0.4s ease' }}>✕</button>
            <h2 style={{ fontSize: '24px', fontWeight: 600, marginBottom: '20px', color: '#1f2937' }}>{getAgentButtonText()}</h2>
            <form onSubmit={handleFormSubmit}>
              {coListingAgents.length > 0 && (
                <div style={{ marginBottom: '1.5rem' }}>
                  <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 500, color: '#374151', marginBottom: '0.5rem' }}>Select Agent</label>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    <button type="button" onClick={() => setFormData({ ...formData, selectedAgent: 'primary' })} style={{ width: '100%', padding: '1rem', borderRadius: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', cursor: 'pointer', textAlign: 'left', border: 'none', background: formData.selectedAgent === 'primary' ? '#1f2937' : '#f3f4f6', color: formData.selectedAgent === 'primary' ? 'white' : '#1f2937', transition: 'all 0.2s' }}>
                      {agent?.profile_image ? <img src={agent.profile_image} alt="" style={{ width: '48px', height: '48px', borderRadius: '50%', objectFit: 'cover' }} /> : <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: formData.selectedAgent === 'primary' ? '#374151' : '#d1d5db', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ color: formData.selectedAgent === 'primary' ? 'white' : '#6b7280' }}>{agent?.name?.charAt(0)}</span></div>}
                    ...
                    {coListingAgents.map((ca: any) => (
                      <button key={ca.id} type="button" onClick={() => setFormData({ ...formData, selectedAgent: ca.id })} style={{ width: '100%', padding: '1rem', borderRadius: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', cursor: 'pointer', textAlign: 'left', border: 'none', background: formData.selectedAgent === ca.id ? '#1f2937' : '#f3f4f6', color: formData.selectedAgent === ca.id ? 'white' : '#1f2937', transition: 'all 0.2s' }}>
                        {ca.profile_image ? <img src={ca.profile_image} alt="" style={{ width: '48px', height: '48px', borderRadius: '50%', objectFit: 'cover' }} /> : <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: formData.selectedAgent === ca.id ? '#374151' : '#d1d5db', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ color: formData.selectedAgent === ca.id ? 'white' : '#6b7280' }}>{ca.name?.charAt(0)}</span></div>}
              ...
              <div style={{ marginBottom: '1rem' }}><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 500, color: '#374151', marginBottom: '0.5rem' }}>Name</label><input type="text" required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} placeholder="Your name" style={{ width: '100%', padding: '0.75rem 1rem', border: '1px solid rgba(215,219,227,0.4)', borderRadius: '0.5rem', fontSize: '1rem', boxSizing: 'border-box' }} /></div>
              <div style={{ marginBottom: '1rem' }}><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 500, color: '#374151', marginBottom: '0.5rem' }}>Email</label><input type="email" required value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} placeholder="your@email.com" style={{ width: '100%', padding: '0.75rem 1rem', border: '1px solid rgba(215,219,227,0.4)', borderRadius: '0.5rem', fontSize: '1rem', boxSizing: 'border-box' }} /></div>
              <div style={{ marginBottom: '1rem' }}><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 500, color: '#374151', marginBottom: '0.5rem' }}>Phone</label><input type="tel" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} placeholder="(555) 123-4567" style={{ width: '100%', padding: '0.75rem 1rem', border: '1px solid rgba(215,219,227,0.4)', borderRadius: '0.5rem', fontSize: '1rem', boxSizing: 'border-box' }} /></div>
              <div style={{ marginBottom: '1.5rem' }}><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 500, color: '#374151', marginBottom: '0.5rem' }}>Message</label><textarea required value={formData.message} onChange={e => setFormData({ ...formData, message: e.target.value })} rows={4} placeholder="I'm interested in this property..." style={{ width: '100%', padding: '0.75rem 1rem', border: '1px solid rgba(215,219,227,0.4)', borderRadius: '0.5rem', fontSize: '1rem', resize: 'vertical', minHeight: '100px', boxSizing: 'border-box' }} /></div>
              {selectedAgentData && (
                <div style={{ padding: '1rem', borderRadius: '0.5rem', background: '#f3f4f6', marginBottom: '1.5rem' }}>
                  <p style={{ fontSize: '0.875rem', fontWeight: 500, marginBottom: '0.5rem', margin: '0 0 0.5rem 0' }}>{coListingAgents.length > 0 ? 'Selected Agent Contact' : 'Contact Information'}</p>
                  {selectedAgentData.email && <a href={`mailto:${selectedAgentData.email}`} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#6b7280', fontSize: '0.875rem', marginBottom: '0.25rem', textDecoration: 'none' }}><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M20 4H4C2.9 4 2.01 4.9 2.01 6L2 18C2 19.1 2.9 20 4 20H20C21.1 20 22 19.1 22 18V6C22 4.9 21.1 4 20 4ZM20 8L12 13L4 8V6L12 11L20 6V8Z"/></svg>{selectedAgentData.email}</a>}
                  {selectedAgentData.phone && <a href={`tel:${selectedAgentData.phone}`} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#6b7280', fontSize: '0.875rem', textDecoration: 'none' }}><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/></svg>{selectedAgentData.phone}</a>}
                </div>
              )}
              <button type="submit" style={{ width: '100%', padding: '1rem', background: '#000', color: '#fff', border: 'none', borderRadius: '0.5rem', fontWeight: 500, fontSize: '1rem', cursor: 'pointer' }}>Send Message</button>
            </form>
          </div>
        </div>

        {/* Tour Modal - Circular Reveal from RIGHT */}
        {showTourModal && <div className="numero-modal-overlay" onClick={() => setShowTourModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 10000 }} />}
        <div className={`numero-tour-panel ${showTourModal ? 'panel-open' : ''}`} style={{ position: 'fixed', top: '50%', right: '50%', transform: 'translate(50%, -50%)', width: '90%', maxWidth: '450px', maxHeight: '90vh', borderRadius: '20px', background: 'rgba(255, 255, 255, 0.98)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', boxShadow: '0 20px 30px -8px rgba(19, 19, 22, 0.15)', clipPath: showTourModal ? 'circle(150% at 100% 0%)' : 'circle(0% at 100% 0%)', transition: 'clip-path 0.7s ease-in-out', zIndex: 10001, overflowY: 'auto', pointerEvents: showTourModal ? 'auto' : 'none' }}>
          <div className="numero-modal-content" style={{ padding: '30px', opacity: showTourModal ? 1 : 0, transition: 'opacity 0.3s ease 0.3s' }}>
            ...
            <h2 style={{ fontSize: '24px', fontWeight: 600, marginBottom: '20px', color: '#1f2937' }}>Schedule a Tour</h2>
            <form onSubmit={handleFormSubmit}>
              <div style={{ marginBottom: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', overflowX: 'auto', paddingBottom: '0.5rem' }}>
                  {getDateOptions().map((d, i) => {
                    ...
                    return (
                      <button key={i} type="button" onClick={() => setSelectedDate(d)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '1rem', borderRadius: '0.75rem', border: '1px solid #e5e7eb', background: isSelected ? '#000' : 'white', color: isSelected ? 'white' : '#1f2937', cursor: 'pointer', minWidth: '80px', flexShrink: 0, transition: 'all 0.2s' }}>
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 500, color: '#374151', marginBottom: '0.5rem' }}>Tour Type</label>
                <div style={{ display: 'flex', gap: '1rem' }}>
                  <button type="button" onClick={() => setFormData({ ...formData, tourType: 'video' })} style={{ flex: 1, padding: '1rem', border: '1px solid rgba(215,219,227,0.4)', borderRadius: '0.5rem', background: formData.tourType === 'video' ? '#000' : 'white', color: formData.tourType === 'video' ? 'white' : '#1f2937', fontWeight: 500, cursor: 'pointer', transition: 'all 0.2s' }}>Video Tour</button>
                  <button type="button" onClick={() => setFormData({ ...formData, tourType: 'live' })} style={{ flex: 1, padding: '1rem', border: '1px solid rgba(215,219,227,0.4)', borderRadius: '0.5rem', background: formData.tourType === 'live' ? '#000' : 'white', color: formData.tourType === 'live' ? 'white' : '#1f2937', fontWeight: 500, cursor: 'pointer', transition: 'all 0.2s' }}>In-Person Tour</button>
                </div>
              </div>
              <div style={{ marginBottom: '1rem' }}><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 500, color: '#374151', marginBottom: '0.5rem' }}>Name</label><input type="text" required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} placeholder="Your name" style={{ width: '100%', padding: '0.75rem 1rem', border: '1px solid rgba(215,219,227,0.4)', borderRadius: '0.5rem', fontSize: '1rem', boxSizing: 'border-box' }} /></div>
              <div style={{ marginBottom: '1rem' }}><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 500, color: '#374151', marginBottom: '0.5rem' }}>Email</label><input type="email" required value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} placeholder="your@email.com" style={{ width: '100%', padding: '0.75rem 1rem', border: '1px solid rgba(215,219,227,0.4)', borderRadius: '0.5rem', fontSize: '1rem', boxSizing: 'border-box' }} /></div>
              <div style={{ marginBottom: '1.5rem' }}><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 500, color: '#374151', marginBottom: '0.5rem' }}>Phone</label><input type="tel" required value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} placeholder="(555) 123-4567" style={{ width: '100%', padding: '0.75rem 1rem', border: '1px solid rgba(215,219,227,0.4)', borderRadius: '0.5rem', fontSize: '1rem', boxSizing: 'border-box' }} /></div>
              <button type="submit" style={{ width: '100%', padding: '1rem', background: '#000', color: '#fff', border: 'none', borderRadius: '0.5rem', fontWeight: 500, fontSize: '1rem', cursor: 'pointer' }}>Request Tour</button>
            </form>
          </div>
        </div>

        {/* Hero Section */}
        <section id="property-hero" className="numero-light-hero pt-16 pb-10 px-8">
              ...
              <div className="pt-16 lg:pt-20">
                <FadeIn delay={0.2} direction="left">
                  <div className="flex flex-wrap items-baseline gap-4 mb-6">
                    <span className="numero-hero-address px-4 py-2 text-lg lg:text-xl font-medium text-white bg-black" style={{ borderRadius: '8px' }}>{property.address?.toUpperCase()}</span>
                ...
                <FadeIn delay={1.8} direction="left">
                  <span className="numero-hero-price inline-block px-5 py-3 text-xl lg:text-2xl font-medium text-white bg-black" style={{ borderRadius: '8px' }}>
        ...
        {heroImage && (
          <section className="numero-light-hero-image px-8" style={{ paddingTop: '60px', paddingBottom: '60px' }}>
            <div className="max-w-[1400px] mx-auto">
        ...
        <section id="the-property-numbers" className="numero-light-numbers px-8 py-20">
          <div className="max-w-[1400px] mx-auto">
            ...
            <div className="numero-numbers-grid hidden lg:grid gap-6" style={{ gridTemplateColumns: '2fr 1fr 1fr 2fr 2fr' }}>
              <FadeIn delay={0.3}>
                <div className="numero-numbers-box p-6 rounded-2xl h-full" style={{ backgroundColor: COLORS.bgWhite }}>
              ...
              <FadeIn delay={0.6}>
                <div className="p-6 rounded-2xl h-full" style={{ backgroundColor: COLORS.bgWhite }}>
                  <p className="text-xs tracking-[0.15em] uppercase mb-2" style={{ color: COLORS.textGray }}>{content?.numbers_box3_label || 'Property Size'}</p>
                  <p className="text-2xl lg:text-3xl font-medium" style={{ color: COLORS.text }}>{content?.numbers_box3_value || `${getSqft().toLocaleString()} sqft`}</p>
                </div>
              </FadeIn>
              <FadeIn delay={0.7}>
                <div className="numero-numbers-box p-6 rounded-2xl h-full" style={{ backgroundColor: COLORS.bgWhite }}>
            ...
            <div className="numero-numbers-grid-mobile grid lg:hidden grid-cols-2 gap-4">
              <FadeIn delay={0.3}>
                <div className="numero-numbers-box p-5 rounded-2xl col-span-2" style={{ backgroundColor: COLORS.bgWhite }}>
                  <p className="numero-numbers-box-label text-xs tracking-[0.15em] uppercase mb-2" style={{ color: COLORS.textGray }}>{content?.numbers_box1_label || 'MLS Number'}</p>
              ...
              <FadeIn delay={0.4}>
                <div className="numero-numbers-box p-5 rounded-2xl" style={{ backgroundColor: COLORS.bgWhite }}>
                  <p className="numero-numbers-box-label text-xs tracking-[0.15em] uppercase mb-2" style={{ color: COLORS.textGray }}>{content?.numbers_box2_label || 'Bedrooms'}</p>
        ...
        {/* The Story Section */}
        <section className="numero-light-story px-8 py-20">
          <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-16">
            ...
            <div>
              ...
              <FadeIn delay={0.5}><div className="numero-property-description text-base lg:text-lg leading-relaxed prose prose-lg max-w-none" style={{ color: COLORS.text }} dangerouslySetInnerHTML={{ __html: content?.property_description || property.public_remarks || property.description || 'A remarkable property featuring exceptional design and premium finishes throughout.' }} /></FadeIn>
            </div>
          </div>
        </section>
        ...
        {images.length > 0 && (
          <section id="property-gallery" className="numero-light-gallery px-8 py-20">
            <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-16">
              ...
              <div>
                ...
                <FadeIn delay={0.4}>
                  <MasonryGallery images={images} onImageClick={(idx) => openLightbox(images, idx)} />
                </FadeIn>
              </div>
            </div>
          </section>
        ...
        {/* Virtual Tour - CONDITIONAL */}
        {property.virtual_tour_url && sectionNums.virtualTour && (
          <section id="virtual-tour-section" className="numero-light-virtual-tour px-8 py-20">
            <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-16">
              ...
              <div>
                ...
                <FadeIn delay={0.4}><div className="numero-virtual-tour-container aspect-video rounded-lg overflow-hidden" style={{ backgroundColor: COLORS.bgWhite }}><iframe src={property.virtual_tour_url} className="w-full h-full" allowFullScreen title="Virtual Tour" /></div></FadeIn>
              </div>
            </div>
          </section>
        )}

        {/* Features & Amenities - CONDITIONAL - redox style: 3-column grid per row (number | category | features) */}
        {sectionNums.features && ((Array.isArray(property.appliances) && property.appliances.length > 0) || (Array.isArray(property.exterior_features) && property.exterior_features.length > 0) || (Array.isArray(property.interior_features) && property.interior_features.length > 0) || (Array.isArray(property.community_features) && property.community_features.length > 0)) ? (
          <section id="property-features" className="numero-light-features px-8 py-20">
            <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-16">
        ...
        {/* Room Dimensions */}
        {/* Room Dimensions - CONDITIONAL */}
        {sectionNums.roomDimensions && property.room_dimensions && (() => {
          try {
            ...
            if (Array.isArray(rooms) && rooms.length > 0) {
              ...
              return (
                <section className="numero-light-room-dimensions px-8 py-20">
                  <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-16">
                      ...
                      <FadeIn delay={0.4}>
                        <div className="numero-room-dimensions-table overflow-x-auto">
                          <table className="w-full text-left">
                            <thead>
                              <tr className="numero-room-dimensions-header" style={{ backgroundColor: '#f5f5f5' }}>
                                <th className="numero-room-dimensions-th py-4 px-6 text-xs tracking-[0.15em] uppercase font-semibold" style={{ color: COLORS.textGray }}>Level</th>
                                <th className="numero-room-dimensions-th py-4 px-6 text-xs tracking-[0.15em] uppercase font-semibold" style={{ color: COLORS.textGray }}>Room</th>
                                <th className="numero-room-dimensions-th py-4 px-6 text-xs tracking-[0.15em] uppercase font-semibold text-right" style={{ color: COLORS.textGray }}>Dimensions</th>
                              </tr>
                            </thead>
                            <tbody>
                        ...
                        {/* Floor Plan CTA */}
                        {floorPlanUrl && (
                          <FloorPlanCTA url={floorPlanUrl} textColor={COLORS.text} />
                        )}
                      </FadeIn>
                    </div>
                  </div>
        ...
        {/* Neighbourhood/City Sections */}
        {hasProfileData && config && (
          <>
            {/* Neighbourhood Hero Banner - Full Width */}
            {config.heroBanner && config.heroBanner.heroImage && (
              <section 
                id="neighbourhood-section" 
                className="numero-light-neighbourhood-hero relative w-full"
                style={{
                ...
                <div className="relative z-10 max-w-[1400px] mx-auto px-8 py-20 flex flex-col justify-end" style={{ minHeight: '600px' }}>
                  <FadeIn delay={0.2}>
                    <p className="numero-neighbourhood-hero-label text-sm tracking-[0.2em] uppercase mb-6 text-white/90">
                      {hasNeighbourhood ? 'Know The Neighbourhood' : 'Know The City'}
                    </p>
                  </FadeIn>
                  <FadeIn delay={0.3}>
                    <h2 className="numero-neighbourhood-hero-headline text-5xl lg:text-7xl font-bold text-white mb-4">
                      {config.heroBanner.headline || profileData?.name}
                    </h2>
                  </FadeIn>
            ...
            {/* Neighbourhood Introduction Section - Dynamic number on left, content on right */}
            {config.neighbourhoodIntro && sectionNums.neighbourhood && (
              <section id="neighbourhood-intro" className="numero-light-neighbourhood-intro px-8 py-20">
                <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-16">
                  ...
                  <div>
                    <FadeIn delay={0.3}>
                      <p className="numero-section-label text-xs tracking-[0.2em] uppercase mb-4" style={{ color: COLORS.textGray }}>{hasNeighbourhood ? 'The Neighbourhood' : 'The City'}</p>
                    </FadeIn>
                    <FadeIn delay={0.4}>
            ...
            {/* Interactive Map - Always show if profile data exists */}
            {sectionNums.map && BACKEND_URL && (
              <section id="neighbourhood-map-section" className="numero-light-map px-8 py-20">
                <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-16">
                  ...
                  <div>
                    ...
                    <FadeIn delay={0.4}><div className="numero-map-container rounded-2xl overflow-hidden" style={{ height: '600px', backgroundColor: COLORS.bgWhite }}><iframe src={`${BACKEND_URL}/embed?id=${profileData?.id}&hideNav=true`} className="w-full h-full" title="Map" style={{ border: 'none' }} /></div></FadeIn>
                  </div>
                </div>
              </section>
            )}
            ...
            {/* Get Outside - Conditional */}
            {sectionNums.outside && config?.outside?.images?.length > 0 && (
              <section className="numero-light-get-outside px-8 py-20">
                <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-16">
            ...
            {/* Local Amenities - Conditional */}
            {sectionNums.amenities && config?.amenities?.images?.length > 0 && (
              <section className="numero-light-amenities px-8 py-20">
                <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-16">
            ...
            {/* Shop & Dine - Conditional */}
            {sectionNums.shopDine && config?.shopDine?.images?.length > 0 && (
              <section className="numero-light-shop-dine px-8 py-20">
                <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-16">
        ...
        {/* Footer */}
        <footer className="numero-light-footer px-8 pt-20 pb-8">
          <div className="max-w-[1400px] mx-auto text-center">
            <p className="text-sm" style={{ color: COLORS.textGray }}>© {new Date().getFullYear()} In The Hood. All rights reserved.</p>
          </div>
        </footer>
